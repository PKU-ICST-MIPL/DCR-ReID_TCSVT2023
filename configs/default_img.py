...
_C.DATA.ROOT = '/data1/cuizhenyu/Simple-CCReID-main/datasets/'
...
_C.DATA.TRAIN_BATCH = 32 #32
# Batch size for testing
_C.DATA.TEST_BATCH = 128 #128
...
_C.PARA = CN()
# Random crop prob
_C.PARA.SHUF_PID_RATIO = 1.0
# Random erase prob
_C.PARA.SHUF_ADV_CLO_RATIO = 0.1
# Random flip prob
_C.PARA.RECON_RATIO = 1.0
# -----------------------------------------------------------------------------
# Model settings
# -----------------------------------------------------------------------------
_C.MODEL = CN()
...
# Hidden reconstruct dim for model
_C.MODEL.HID_REC_DIM = 4096
# No cLoth dim for model
_C.MODEL.NO_CLOTHES_DIM = 2048
# Contour dim for model
_C.MODEL.CONTOUR_DIM = 1024
# CLoth dim for model
_C.MODEL.CLOTHES_DIM = 1024
# -----------------------------------------------------------------------------
# Losses for training 
# -----------------------------------------------------------------------------
_C.LOSS = CN()
...
def update_config(config, args):
    ...
    config.PARA.SHUF_PID_RATIO = args.spr
    config.PARA.SHUF_ADV_CLO_RATIO = args.sacr
    config.PARA.RECON_RATIO = args.rr
    # output folder
    config.OUTPUT = os.path.join(config.OUTPUT, config.DATA.DATASET, config.TAG, str(config.PARA.SHUF_PID_RATIO)+'-'+str(config.PARA.SHUF_ADV_CLO_RATIO)+'-'+str(config.PARA.RECON_RATIO))
