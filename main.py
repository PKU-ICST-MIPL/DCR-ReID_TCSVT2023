...
def parse_option():
    ...
    # Miscs
    parser.add_argument('--spr', default = '1.0', type = float)
    parser.add_argument('--sacr', default = '0.05', type = float)
    parser.add_argument('--rr', default = '1.0', type = float)
...
def main(config):
    ...
    criterion_cla, criterion_pair, criterion_clothes, criterion_adv, criterion_shuffle, recon_uncloth, recon_contour, recon_cloth = build_losses(config, dataset.num_train_clothes)
    # Build optimizer
    ...
    if config.EVAL_MODE:
        ...
        with torch.no_grad():
            if config.DATA.DATASET == 'prcc':
                test_prcc(config, model, queryloader_same, queryloader_diff, galleryloader, dataset)
            else:
    ...
    for epoch in range(start_epoch, config.TRAIN.MAX_EPOCH):
        ...
        if (epoch+1) > config.TEST.START_EVAL and config.TEST.EVAL_STEP > 0 and \
            (epoch+1) % config.TEST.EVAL_STEP == 0 or (epoch+1) == config.TRAIN.MAX_EPOCH or (epoch+1) >= 30:
            logger.info("==> Test")
            torch.cuda.empty_cache()
            if config.DATA.DATASET == 'prcc':
                rank1, mAP = test_prcc(config, model, queryloader_same, queryloader_diff, galleryloader, dataset)
            else:
