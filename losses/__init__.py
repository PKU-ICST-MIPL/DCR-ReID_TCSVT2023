...
def build_losses(config, num_train_clothes):
    ...
    if config.LOSS.CAL == 'cal':
        ...
        criterion_shuffle = ClothesBasedAdversarialLoss(scale=config.LOSS.CLA_S, epsilon=1.0)
        ...
        criterion_shuffle = ClothesBasedAdversarialLossWithMemoryBank(num_clothes=num_train_clothes, feat_dim=config.MODEL.FEATURE_DIM,
                             momentum=config.LOSS.MOMENTUM, scale=config.LOSS.CLA_S, epsilon=1.0)
    else:
        raise KeyError("Invalid clothing classification loss: '{}'".format(config.LOSS.CAL))

    recon_uncloth = nn.L1Loss()
    recon_contour = nn.L1Loss()
    recon_cloth = nn.L1Loss()

    return criterion_cla, criterion_pair, criterion_clothes, criterion_cal, criterion_shuffle, recon_uncloth, recon_contour, recon_cloth
